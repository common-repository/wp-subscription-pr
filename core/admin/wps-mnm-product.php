			 ...
			 woocommerce_wp_text_input(
			  array(
			   ...
			   'description'       => __( 'According to 2Checkout map your Name here, otherwise it will not work.', 'woocommerce' ),
			    'value'       => get_post_meta( $post->ID, 'wps_mnm_subscription_product_name_field', true ),
			   'type'              => 'text'
			   ));
			...
			woocommerce_wp_text_input(
			  array(
			   ...
			   'description'       => __( 'According to 2Checkout map your product id here, otherwise it will not work.', 'woocommerce' ),
			    'value'       => get_post_meta( $post->ID, 'wps_mnm_subscription_product_id_field', true ),
			   'type'              => 'text'
			   ));
			 ...
			 woocommerce_wp_text_input(
			  array(
			   ...
			   'description'       => __( 'According to 2Checkout map your Startup Fees here, otherwise it will not work.', 'woocommerce' ),
			    'value'       => get_post_meta( $post->ID, 'wps_mnm_subscription_startup_fees_field', true ),
			   'type'              => 'text'
			   ));
			  ...
			  array(
			   ...
			   'value'       => get_post_meta( $post->ID, 'wps_mnm_subscription_quantity_field', true ),
			   'type'              => 'text'
			   ));
			  ...
			  array(
			   ...
			   'value'       => get_post_meta( $post->ID, 'wps_mnm_subscription_2checkout_price_field', true ),
			   'type'              => 'text'
			   ));
			  ...
			  array(
			   ...
			   'value'       => get_post_meta( $post->ID, 'wps_mnm_subscription_description_field', true ),
			   'type'              => 'text'
			   ));
		...
		?><div class='options_group'><?php
			...
			woocommerce_wp_text_input(
			  array(
			   ...
			   'desc_tip'    => 'true',
			   'description'       => __( 'According to Stripe map your plan id here, otherwise it will not work.', 'woocommerce' ),
			    'value'       => get_post_meta( $post->ID, 'wps_mnm_subscription_plan_field', true ),
			   'type'              => 'text'
			   ));
			 ...
			 woocommerce_wp_text_input(
			  array(
			   ...
			   'value'       => get_post_meta( $post->ID, 'wps_mnm_subscription_stripe_price_field', true ),
			   'type'              => 'text'
			   ));
		?></div>
...
add_action('admin_footer', 'wps_mnm_payment_xyz');
function wps_mnm_payment_xyz(){?>
<!--<script type="text/javascript">
jQuery(function($) {
    $(".show_if_spwcsubscription").click(function() {
        //alert('yes');
         $("#notice-select-method").hide();
		var scribe_pay_method = $(".wps_mnm_method_notes").val();		
		//alert(scribe_pay_method);
		 if (scribe_pay_method == 0) 
		 {
            //alert("Please choose some option");
            $("#notice-select-method").show();
		 } 
		 else
		 {
			 $("#plz-select-payment-method").hide();
		 }
    });
});	
</script>-->
	 ...
	 //alert('ok');
		$('.wps_mnm_method_notes').on('change', function() {
			...
			if (scribe_pay_method == 0) 
			 {
				...
				$("#notice-twocheckout").hide();
				$("#notice-stripe").hide();
				$("#notice-authorize").hide();
			 } 
			if(scribe_pay_method == 'wps_mnm_auth_net'){
				$("#notice-authorize").show().delay(15000).fadeOut();
				$("#notice-twocheckout").hide();
				$("#notice-stripe").hide();
				$("#notice-select-method").hide();
			...
			if(scribe_pay_method == 'wps_mnm_stripe'){
				$("#notice-stripe").show().delay(15000).fadeOut();
				$("#notice-twocheckout").hide();
				$("#notice-authorize").hide();
				$("#notice-select-method").hide();
			...
			if(scribe_pay_method == 'twocheckout'){
				$("#notice-twocheckout").show().delay(15000).fadeOut();
				$("#notice-stripe").hide();
				$("#notice-authorize").hide();
				$("#notice-select-method").hide();
...
function rental_options_product_tab_content() {
	...
	?><div id='rental_options' class='panel woocommerce_options_panel'><?php
		...
		$beginning = 'Select';
		$result = array_merge((array)$beginning, $b);
		//print_r($result);
		...
		//echo '<pre>';print_r($c);echo '</pre>';
		 ...
		 woocommerce_wp_select( 
			...
			?>
			<p class="notice_select" id="notice-select-method" style="display:none; color:red;font-size: 15px;">
			Please select any one payment method.</p>
			
			<p class="notice_authorize" id="notice-authorize" style="display:none; color:blue;font-size: 18px;">
			If Payment Method is Authorize . Net,Please select Subscription Renewal Length Unit as Days or Months.</p>
			...
			/*woocommerce_wp_text_input(
			  array(
			   ...
			   /*echo '<b style="font-size:13px; color:blue;">If Payment Method is Authorize . Net,Please select Subscription Renewal Length Unit as Days or Months. </b>';*/
				?>
