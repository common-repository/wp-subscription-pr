...
class AuthnetARB
{
    ...
    private $interval_length;
    private $interval_unit;
    private $total_occurrences;
    ...
    public function __construct($login, $transkey, $interval_length, $interval_unit, $total_occurrences, $test = self::USE_PRODUCTION_SERVER)
    {
		/*echo $interval_length;
		echo $interval_unit;
		echo $total_occurrences; die();*/
        ...
        if ($interval_length < 7 && $interval_unit == 'days')
        {
            throw new AuthnetARBException('Please choose subscription renewal length more than 7 days, This is not supported by this payment method');
        }
        if ($interval_unit == 'week' || $interval_unit == 'years')
        {
            throw new AuthnetARBException('Interval length is not supported by this payment method.');
        }
        ...
        //$this->params['interval_length']  = 7;
        $this->params['interval_length']  = $interval_length;
        //$this->params['interval_unit']    = 'days';
        $this->params['interval_unit']    = $interval_unit;
        //$this->params['startDate']        = date("Y-m-d", strtotime("+ 7 day"));
        if($interval_unit == 'days'){
			$this->params['startDate']        = date("Y-m-d", strtotime('+' .$interval_length. 'day'));
		}
		if($interval_unit == 'months'){
			$this->params['startDate']        = date("Y-m-d", strtotime('+' .$interval_length. 'months'));
		}
		// Week and Year does not supported by Authorize . Net
		/*if($interval_unit == 'week'){
			$this->params['startDate']        = date("Y-m-d", strtotime('+' .$interval_length. 'week'));
		}
		if($interval_unit == 'years'){
			$this->params['startDate']        = date("Y-m-d", strtotime('+' .$interval_length. 'years'));
		}*/
        //$this->params['totalOccurrences'] = 9999;
        $this->params['totalOccurrences'] = $total_occurrences;
    ...
    private function process()
    {
        ...
        echo "Response==>".print_r($this->response);
        if($this->response)
        {
            $this->parseResults();
            //echo "Perse==>".print_r($this->parseResults());
